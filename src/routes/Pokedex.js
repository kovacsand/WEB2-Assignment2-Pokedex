...
function Pokedex() {
	...
	const loadSelectedPokemon = (pokemon) => {
		...
		about.innerHTML = `Height: ${pokemon.height * 10} cm Weight: ${
			pokemon.weight / 10
		} kg`;
	...
	return <PokedexGrid arrowFunction={changePage} pageNumber={pageNumber} />;
}
...
function PokedexGrid({ arrowFunction, pageNumber, showAbout }) {
	return (
		<>
			<div id="pokedex">
				<LeftPanel />
				<RightPanel arrowFunction={arrowFunction} pageNumber={pageNumber} />
			</div>
		</>
	);
}
...
function RightPanel({ arrowFunction, pageNumber }) {
	return (
		<>
			<div id="right-panel">
				{/* Blank container */}
					...
					<div className="center-left-container">
						...
						<div className="white-squares-container">
							<div
								...
								onClick={() => arrowFunction("previous")}
							>
								<i className="arrow facing-left"></i>
							</div>
							<div
								className="white-square"
								onClick={() => arrowFunction("next")}
							>
								<i className="arrow facing-right"></i>
							</div>
						</div>
					</div>
